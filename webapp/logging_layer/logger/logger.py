    ...
    def get_log(self, project_id, execution_id,process_type=None, data_time_value=None):
        try:
            ...
            if process_type=="training":
                database_name = Initializer().get_training_database_name()
            else:
                database_name = Initializer().get_prediction_database_name()
            database_name="training_pipeline"
            ...
            required_collection_list=['pipeline']
            yield '...<br>'
            ...
            while is_running:
                ...
                if data_time_value is not None:
                    query = {'updated_date_and_time': {"$gt": parse(data_time_value)},
                ...
                for collection in required_collection_list:
                    ...
                    for res in result:
                        ...
                        log_data = "</br><span style='color:green'>log_updated_date [{}]</span><span style='color:#008cba;'> log_update_time " \
                                   "{}:</span> <span style='color:purple;'>{}".format(    res['log_updated_date'],
                                                                                                   res['log_update_time'],
                                                                                                   res['message'])
                ...
                result = MongoDBOperation().get_record(database_name=database_name_thread, collection_name=collection_name_thread,
                                                       query=query)
                if result is None:
